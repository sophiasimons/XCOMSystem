...
#define DEFAULT_TX_STM32_IP "192.168.0.10"
...
int main(int argc, char *argv[]) {
    ...
    int loop_count = 1;  // Default: send once
    ...
    if (all_passed) {
        ...
        // Loop sending based on loop_count parameter
        bool infinite_loop = (loop_count == 0);
        int iteration = 0;
        
        while (infinite_loop || iteration < loop_count) {
            iteration++;
            
            if (loop_count > 1 || infinite_loop) {
                printf("\n--- Iteration %d %s---\n", iteration, 
                       infinite_loop ? "(Press Ctrl+C to stop) " : "");
            }
            ...
            // Wait between iterations (except on last one)
            if (infinite_loop || iteration < loop_count) {
                printf("  Waiting 2 seconds before next send...\n");
                sleep(2);
            }
        }
    } else {
    ...
    printf("Usage: %s [tx_ip] [port] [loop_count]\n", argv[0]);
    printf("Example: %s %s %d 3  (send 3 times)\n", argv[0], tx_stm32_ip, port);
    printf("         %s %s %d 0  (infinite loop)\n\n", argv[0], tx_stm32_ip, port);
